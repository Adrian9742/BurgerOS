; ── FlameOS — Script NSIS customizado ────────────────────────────────────────
...
!macro customInstall
  ...
  ${If} $0 != 0
    ...
    burgeros_download_python:
      ...
      FileOpen $9 "$TEMP\flameos_getpython.ps1" w
      FileWrite $9 "try {"
      ...
      nsExec::ExecToLog 'powershell.exe -ExecutionPolicy Bypass -NonInteractive -File "$TEMP\flameos_getpython.ps1"'
      Pop $2
      Delete "$TEMP\flameos_getpython.ps1"
    ...
    burgeros_skip_python:
      MessageBox MB_OK|MB_ICONINFORMATION \
        "Python não será instalado.$\nO FlameOS não funcionará sem o Python.$\nInstale em: python.org/downloads e reinstale o FlameOS."
      Goto burgeros_python_done
  ${EndIf}
  ...
  ${Else}
    MessageBox MB_OK|MB_ICONEXCLAMATION \
      "Falha ao instalar dependências Python.$\nO FlameOS tentará instalá-las na primeira execução automaticamente."
  ${EndIf}
